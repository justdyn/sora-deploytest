...
interface ReservationsProps extends PageProps {
    reservations: Reservation[];
}

export default function Reservations({ auth, reservations }: ReservationsProps) {
    const openWhatsApp = (phoneNumber: string, customerName: string) => {
        // Format phone number (remove any non-digit characters and ensure it starts with 62)
        const formattedNumber = phoneNumber.replace(/\D/g, '').replace(/^0+/, '62');
                                        ...
                                        {reservations.map((reservation: Reservation) => (
                                            <tr key={reservation.id} className="border-b">
                                                ...
                                                <td className="px-4 py-2">
                                                    <button
                                                        onClick={() => openWhatsApp(reservation.pelanggan.no_telepon, reservation.pelanggan.name)}
